...
@Disabled
@Autonomous
class NewMidRightAuto : RogueBaseAuto() {
...
//        .turn(-49-90)
    ...
    private fun Anvil.goToDeposit(it: Int) = when (it) {
        0 -> splineTo(78 + poleOffset.x, -10.5 + poleOffset.y, -25-90)
        1 -> splineTo(78 + poleOffset.x, -10.5 + poleOffset.y, -25-90)
        2 -> splineTo(78 + poleOffset.x, -10.5 + poleOffset.y, -25-90)
        3 -> splineTo(78 + poleOffset.x, -10.5 + poleOffset.y, -25-90)
        4 -> splineTo(78 + poleOffset.x, -10.5 + poleOffset.y, -25-90)
        else -> throw CycleException()
    }

    private fun Anvil.goToIntake(it: Int) = when (it) {
        0 -> splineTo(180, 6, 0)
        1 -> splineTo(180.2, 8.5, 0)
        2 -> splineTo(180.1, 9, 0)
        3 -> splineTo(180, 11, 0)
        4 -> splineTo(179.3, 13, 0)
        else -> throw CycleException()
    }.doInReverse()
...
//            bot.intake.enable()
    ...
    private fun parkTraj(startPose: Pose2d) =
        Anvil.forgeTrajectory(bot.drive, startPose) {
            ...
            when (signalID) {
                1 -> {
                    lineToLinearHeading(2.5, 37.5, 180)
                }
                2 -> {
                    lineToLinearHeading(92.5, 37.5, 180)
                }
                3 -> {
                    lineToLinearHeading(160, 37.5, 180)
                }
            }
