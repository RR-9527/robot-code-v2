...
<TabItem value="kt" label="Kotlin">
  ...
  override fun runOpMode() {
      ...
      Scheduler.debug(this) {
          telemetry.addData("LoopTime (ms)", it.loopTime)
          telemetry.addData("Number of hooked listeners", it.numHookedListeners)
          telemetry.addData("Number of (unique) msg subscribers", it.numUniqueMessageSubs)
          telemetry.update()
      }
  }
  ```
...
Optionally, if you want to only nuke specific parts of the Scheduler, you can pass in `Nuke.Listeners`,
`Nuke.Messages`, or `Nuke.BeforeEach` (you can pass in multiple by or-ing them together)
...
<TabItem value="java" label="Java">
  ...
  public void runOpMode() throws InterruptedException {
      ...
      Scheduler.nuke(Nuke.Listeners) // Only nukes the listeners

      Scheduler.nuke(Nuke.Listeners | Nuke.BeforeEach) // Nukes everything besides messages
  }
  ```
...
<TabItem value="kt" label="Kotlin">
  ...
  override fun runOpMode() {
      ...
      Scheduler.nuke(Listeners or BeforeEach) // Nukes everything besides messages
  }
  ```
